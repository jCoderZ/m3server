 ...
 * @author mrumpf
 */
public class UpnpProtocolAdapter extends ProtocolAdapter implements
		RegistryListener {

	private static final Logger logger = Logging
			.getLogger(UpnpProtocolAdapter.class);
	...
	public void startup() {
		if (logger.isLoggable(Level.FINE)) {
			logger.fine("Startup of UpnpProtocolAdapter...");
		}
	...
	public void shutdown() {
		if (logger.isLoggable(Level.FINE)) {
			logger.fine("Shutdown of UpnpProtocolAdapter...");
		}
	...
	public void remoteDeviceAdded(Registry registry, RemoteDevice device) {
		logger.log(Level.INFO, "Remote device available: {0} ({1})",
				new Object[] { device.getDetails().getFriendlyName(),
						device.getType().getType() });
		// we deal with MediaRenderer devices only
		if (MEDIA_RENDERER.equals(device.getType().getType())) {
			logger.log(Level.FINER, "Remote device details: {0} ",
					ToStringBuilder.reflectionToString(device));

			// TODO: Find out the device capabilities and add those as
			// attributes to the renderer
			Renderer r = RendererRegistry.findRenderer(device.getDetails()
					.getFriendlyName());
			if (r == null) {
