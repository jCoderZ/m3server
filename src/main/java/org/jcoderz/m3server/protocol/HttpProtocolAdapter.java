...
public class HttpProtocolAdapter extends ProtocolAdapter {
    ...
    public void startup() {
        try {
            ...
            httpServer = GrizzlyServerFactory.createHttpServer(UriBuilder.fromUri(
                    getString(HTTP_PROTOCOL_KEY) + "://"
                    + getString(HTTP_HOSTNAME_KEY) + "/"
                    + getString(HTTP_REST_SERVICES_ROOT_CONTEXT_KEY)).port(getInteger(HTTP_PORT_KEY)).build(), rc);
            httpServer.getServerConfiguration().addHttpHandler(new ClasspathHttpHandler());
